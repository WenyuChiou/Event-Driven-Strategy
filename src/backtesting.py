...
def compare_strategies_with_kd(data, long_threshold=0.005971, short_threshold=0.002471, 
                         k_period=14, d_period=3, overbought=80, oversold=20,
                         holding_period=3, title=None, save_path=None):
    """
    Compare a probability threshold strategy with a KD indicator strategy
    
    Parameters:
    -----------
    data : pd.DataFrame
        DataFrame containing the data including 'high', 'low', 'close', etc.
    ...
    holding_period : int, default=3
        Holding period for both strategies
    title : str, optional
        Chart title
    save_path : str, optional
    ...
    --------
    tuple
        (prob_backtester, kd_backtester) containing both backtester instances
    """
    # Create strategies
    prob_strategy = ProbabilityThresholdStrategy(
        long_threshold=long_threshold,
        short_threshold=short_threshold,
        holding_period=holding_period,
        name=f"Probability (L:{long_threshold:.6f}, S:{short_threshold:.6f})"
    )
    ...
    kd_strategy = KDStrategy(
        ...
        oversold=oversold,
        holding_period=holding_period,
        name=f"KD (K:{k_period}, D:{d_period}, OB:{overbought}, OS:{oversold})"
    )
    
    # Create backtesters
    prob_backtester = Backtester(profit_loss_window=holding_period)
    kd_backtester = Backtester(profit_loss_window=holding_period)
    
    # Run backtest for both strategies
    prob_backtester.run(data, prob_strategy)
    kd_backtester.run(data, kd_strategy)
    
    # Compare strategies
    prob_backtester.compare_with_strategy(
        kd_backtester, 
        title=title or "Probability Strategy vs KD Strategy",
        save_path=save_path
    )
    
    # Print metrics comparison
    prob_metrics = prob_backtester.calculate_metrics()
    kd_metrics = kd_backtester.calculate_metrics()
    
    print("===== Probability Strategy Metrics =====")
    ...
    return prob_backtester, kd_backtester
